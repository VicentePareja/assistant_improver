...
BASE_MODEL_NAME = p.BASE_MODEL_NAME
BASE_MODEL_SUFFIX = p.BASE_MODEL_SUFFIX
BASE_TEMPERATURE = p.BASE_TEMPERATURE
BASE_TOP_P = p.BASE_TOP_P

# Fine-tuning
NUM_WORST_EXAMPLES = p.NUM_WORST_EXAMPLES
FINE_TUNED_MODEL_SUFFIX = p.FINE_TUNED_MODEL_SUFFIX

# Evaluator
EVALUATOR_MODEL_NAME = p.EVALUATOR_MODEL_NAME
EVALUATOR_TEMPERATURE = p.EVALUATOR_TEMPERATURE
EVALUATOR_TOP_P = p.EVALUATOR_TOP_P
EVALUATOR_INTRODUCTION_PROMT = p.EVALUATOR_INTRODUCTION_PROMT
EVALUATOR_DESCRIPTION_PROMPT = p.EVALUATOR_DESCRIPTION_PROMPT

# CSV columns
COLUMN_QUESTION = p.COLUMN_QUESTION
COLUMN_HUMAN_ANSWER = p.COLUMN_HUMAN_ANSWER

# Instructions/Examples
PATH_INSTRUCTIONS_TXT = p.PATH_INSTRUCTIONS_TXT
PATH_INSTRUCTIONS_NO_EXAMPLES = p.PATH_INSTRUCTIONS_NO_EXAMPLES
PATH_EXAMPLES_TXT = p.PATH_EXAMPLES_TXT

# Assistant IDs
PATH_ASSISTANTS_IDS_TXT = p.PATH_ASSISTANTS_IDS_TXT
PATH_ASSISTANT_ID_FINE_TUNED_TXT = p.PATH_ASSISTANT_ID_FINE_TUNED_TXT
PATH_INSTRUCTIONS_EVALUATOR_TXT = p.PATH_INSTRUCTIONS_EVALUATOR_TXT
PATH_EVALUATOR_ID_TXT = p.PATH_EVALUATOR_ID_TXT

# Test inputs
PATH_TEST_EXAMPLES_CSV = p.PATH_TEST_EXAMPLES_CSV

# Separate answers & grades
PATH_BASE_ANSWERS_CSV = p.PATH_BASE_ANSWERS_CSV
PATH_BASE_GRADES_CSV = p.PATH_BASE_GRADES_CSV
PATH_FINE_TUNED_ANSWERS_CSV = p.PATH_FINE_TUNED_ANSWERS_CSV
PATH_FINE_TUNED_GRADES_CSV = p.PATH_FINE_TUNED_GRADES_CSV
PATH_UNIFIED_RESULTS_CSV = p.PATH_UNIFIED_RESULTS_CSV

# Worst Qs
PATH_WORST_QUESTIONS_TXT = p.PATH_WORST_QUESTIONS_TXT
PATH_WORST_QUESTIONS_JSONL = p.PATH_WORST_QUESTIONS_JSONL



class AssistantImprover:
    def __init__(self):
        load_dotenv()
    ...
    def create_eval_prompt(self):

        with open(self.path_instructions_no_examples, "r", encoding="utf-8") as f:
            examples_text = f.read()

        with open(self.path_instructions_evaluator_txt, "w", encoding="utf-8") as f:
            f.write(f"{EVALUATOR_INTRODUCTION_PROMT}\n\n")
            f.write(examples_text)
            f.write(f"\n\n{EVALUATOR_DESCRIPTION_PROMPT}")

        print(f"Evaluator assistant prompt created at: {self.path_instructions_evaluator_txt}")
